    ...
    <div style={{minHeight:"100vh",background:"#0d1117",color:"#e8eaf0",fontFamily:"DM Sans,Segoe UI,sans-serif",paddingBottom:60}}>
      ...
      {showExport&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"flex-end",zIndex:300}} onClick={()=>setShowExport(false)}>
          <div style={{background:"#151c2c",borderRadius:"20px 20px 0 0",padding:20,width:"100%",border:"1px solid #1e2a3a",boxSizing:"border-box"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontWeight:700,fontSize:15,marginBottom:6}}>Export CSV</div>
            <div style={{fontSize:11,color:"#556677",marginBottom:16,lineHeight:1.5}}>
              Downloads a CSV file you can open directly in Excel or Google Sheets.
            </div>
            <button onClick={()=>{
              const csv = buildCSV();
              ...
              a.href = url;
              a.download = `nutrition-${getTodayKey()}.csv`;
              document.body.appendChild(a);
              a.click();
              document.body.removeChild(a);
              URL.revokeObjectURL(url);
              setShowExport(false);
            }} style={{width:"100%",background:"#34d399",border:"none",borderRadius:10,color:"#0d1117",padding:13,cursor:"pointer",fontSize:14,fontWeight:700,marginBottom:10}}>
              ↓ Download CSV
            </button>
            <button onClick={()=>setShowExport(false)} style={{width:"100%",background:"#1e2a3a",border:"1px solid #2a3a50",borderRadius:10,color:"#8899aa",padding:12,cursor:"pointer",fontSize:14}}>Close</button>
          </div>
        </div>
      )}
      <style>{`
